...
def flash_1d_kmeans_n_cluster(
    ...
    centroids = _kmeans_plusplus(X, n_clusters,
...
@numba.njit(cache=True)
def _rand_choice_prefix_sum(arr, prob_prefix_sum, start_idx=0, stop_idx=None):
    if stop_idx is None:
        stop_idx = len(arr)
    total_prob = query_prefix_sum(prob_prefix_sum, start_idx, stop_idx)
    selector = np.random.random_sample() * total_prob
    return arr[np.clip(np.searchsorted(prob_prefix_sum, selector), start_idx, stop_idx - 1)]


@numba.njit(cache=True)
def _calculate_inertia(X, centroids, weights_prefix_sum, weighted_X_prefix_sum, weighted_X_squared_prefix_sum,
                       count=None):
...
def _rand_choice_centroids(X, centroids, weights_prefix_sum, weighted_X_prefix_sum, weighted_X_squared_prefix_sum,
                           size):
    total_inertia = _calculate_inertia(X, centroids, weights_prefix_sum, weighted_X_prefix_sum,
                                       weighted_X_squared_prefix_sum)
    ...
    for i in range(size):
        ...
        while left < right:
            mid = (left + right) // 2
            inertia = _calculate_inertia(X, centroids, weights_prefix_sum, weighted_X_prefix_sum,
                                         weighted_X_squared_prefix_sum,
                                         count=mid)
            if inertia < selector:
                left = mid + 1
...
def _kmeans_plusplus(X, n_clusters, weights_prefix_sum, weighted_X_prefix_sum, weighted_X_squared_prefix_sum,
    ...
    for c_id in range(1, n_clusters):
        ...
        for i in range(len(centroid_candidates)):
            ...
            inertia = _calculate_inertia(X, centroids[:c_id + 1], weights_prefix_sum, weighted_X_prefix_sum,
                                         weighted_X_squared_prefix_sum)
